...
function hasDangerousKeys(obj: unknown): boolean {
  ...
  for (const key of Object.keys(obj)) {
    ...
    if (hasDangerousKeys((obj as any)[key])) return true;
  }
  return false;
}
...
const TaskBehaviorSettingsSchema = z.object({
  ...
  attachmentMaxFileSize: z.number().int().min(1024).max(100*1024*1024).optional(),
  attachmentMaxPerTask: z.number().int().min(1).max(100).optional(),
  attachmentMaxTotalSize: z.number().int().min(1024).max(500*1024*1024).optional(),
  enableComments: z.boolean().optional(),
  defaultPriority: z.enum(['none','low','medium','high','critical']).optional(),
}).strict().optional();

const AgentBehaviorSettingsSchema = z.object({
